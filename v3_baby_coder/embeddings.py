...
from llama import llm_embedding
...
class Embeddings:
    ...
    def compute_repository_embeddings(self):
        ...
        df = pd.read_csv(
            os.path.join(self.workspace_path, "playground_data/repository_info.csv")
        )
        ...
        self.save_doc_embeddings_to_csv(
            context_embeddings,
            df,
            os.path.join(self.workspace_path, "playground_data/doc_embeddings.csv"),
        )
    ...
    def get_embedding(self, text: str) -> list[float]:
        result = llm_embedding.create_embedding(input=text)
        result_embedding = result["data"][0]["embedding"][0]
