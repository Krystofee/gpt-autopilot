...
def code_tasks_initializer_agent(objective: str):
    ...
    return llm_call(prompt, temperature=0.4, max_tokens=8192)
...
def code_tasks_refactor_agent(objective: str, task_list_json):
    ...
    return llm_call(prompt, temperature=0.1, max_tokens=8192)
...
def code_tasks_details_agent(objective: str, task_list_json):
    ...
    RETURN JSON ONLY:"""
    return llm_call(prompt, temperature=0, max_tokens=8192)
...
def task_assigner_recommendation_agent(objective: str, task: str):
    ...
    - Implementing new functionality or writing new code typically requires the code_refactor_agent.
    ...
    - If the task involves modifying or optimizing existing code, even writign new functionality to existing class or file, consider using the code_refactor_agent.
...
def task_assigner_agent(objective: str, task: str, recommendation: str):
    ...
    TLDR: To create files, use command_executor_agent, to write text/code to new files, use code_writer_agent, to write code to existing file or to refactor existing code, use code_refactor_agent.
...
def code_writer_agent(task: str, isolated_context: str, context_code_chunks):
    ...
    return extract_python_code(llm_call(prompt, temperature=0, max_tokens=8192))
...
def code_relevance_agent(objective: str, task_description: str, code_chunk: str):
    ...
    relevance_score = llm_call(prompt, temperature=0.5, max_tokens=50)
...
def task_human_input_agent(task: str, human_feedback: str):
    ...
    return llm_call(prompt, temperature=0.3, max_tokens=200)
...
# # Task list
# task_json = json.loads(extract_python_code(task_agent_output))

task_json = {
    "tasks": [
        {
            "id": 1,
            "description": "Run a command to create the project directory named 'snake_game'. Use the command 'mkdir snake_game'.",
            "file_path": "./snake_game"
        },
        {
            "id": 2,
            "description": "Run a command to create an empty Python file named 'snake.py' in the 'snake_game' directory. Use the command 'touch snake_game/snake.py'.",
            "file_path": "./snake_game/snake.py"
        },
        {
            "id": 3,
            "description": "Run a command to create an empty Python file named 'game.py' in the 'snake_game' directory. Use the command 'touch snake_game/game.py'.",
            "file_path": "./snake_game/game.py"
        },
        {
            "id": 4,
            "description": "Run a command to create an empty Python file named 'main.py' in the 'snake_game' directory. Use the command 'touch snake_game/main.py'.",
            "file_path": "./snake_game/main.py"
        },
        {
            "id": 5,
            "description": "Write code to create a 'Snake' class in 'snake.py' with the following methods: '__init__(self, initial_position)' to initialize the snake with a starting position, 'move(self, direction)' to move the snake in a given direction ('W', 'A', 'S', 'D' for up, left, down, and right), 'grow(self)' to increase the length of the snake when it eats an item, 'check_collision(self, game_width, game_height)' to check if the snake has collided with itself or the game boundaries, and 'draw(self, screen)' to draw the snake on the game screen using the 'curses' library. The 'Snake' class should also have attributes to store the snake's position, direction, and body segments. Example usage of the class: \n\n```python\nclass Snake:\n    def __init__(self, initial_position):\n        self.body = [initial_position]\n        self.direction = 'RIGHT'\n\n    def move(self, direction):\n        # Implementation here...\n\n    def grow(self):\n        # Implementation here...\n\n    def check_collision(self, game_width, game_height):\n        # Implementation here...\n\n    def draw(self, screen):\n        # Implementation here...\n```",
            "file_path": "./snake_game/snake.py"
        },
        {
            "id": 6,
            "description": "Write code to create a 'Game' class in 'game.py' with the following methods: '__init__(self, width, height)' to initialize the game with a given width and height, 'start(self)' to start the game loop, 'update(self)' to update the game state in each iteration of the game loop, and 'draw(self)' to draw the game elements on the screen using the 'curses' library. The 'Game' class should also have attributes to store the game screen, snake object, item position, score, and game over state. Example usage of the class: \n\n```python\nclass Game:\n    def __init__(self, width, height):\n        self.width = width\n        self.height = height\n        self.snake = Snake((width // 2, height // 2))\n        self.item_position = self.generate_random_item_position()\n        self.score = 0\n        self.game_over = False\n\n    def start(self):\n        # Implementation here...\n\n    def update(self):\n        # Implementation here...\n\n    def draw(self):\n        # Implementation here...\n\n    def generate_random_item_position(self):\n        # Implementation here...\n```",
            "file_path": "./snake_game/game.py"
        },
        {
            "id": 7,
            "description": "Write code in 'main.py' to define a 'main()' function that initializes a 'Game' object with a specified width and height (e.g., width=20, height=20), starts the game by calling the 'start()' method of the 'Game' object, and handles user input for controlling the snake's movement using the 'curses' library. The 'main()' function should also handle the game over condition and display the final score. Example code: \n\n```python\nimport curses\nfrom game import Game\n\nWIDTH, HEIGHT = 20, 20\n\ndef main():\n    screen = curses.initscr()\n    curses.curs_set(0)\n    game = Game(WIDTH, HEIGHT)\n    try:\n        game.start()\n    except KeyboardInterrupt:\n        curses.endwin()\n        print(f'Final Score: {game.score}')\n\nif __name__ == '__main__':\n    main()\n```",
            "file_path": "./snake_game/main.py"
        },
        {
            "id": 8,
            "description": "Edit the code in 'main.py' to add a conditional block that checks if the script is being run as the main program, and if so, calls the 'main()' function to start the game. Add this code to the end of 'main.py': \n\n```python\nif __name__ == '__main__':\n    main()\n```",
            "file_path": "./snake_game/main.py"
        },
        {
            "id": 9,
            "description": "Run a command to execute the 'main.py' script to start the Snake game. Use the command 'python snake_game/main.py'.",
            "file_path": "./snake_game/main.py"
        }
    ]
}
...
for task in task_json["tasks"]:
    ...
    if chosen_agent == "command_executor_agent":
        ...
        if chosen_agent == "code_writer_agent":
            ...
            print("[RELEVANT CHUNKS] START")
            print(relevant_chunks)
            print("[RELEVANT CHUNKS] END")
